...
class TestTripSearchCarrierFilterAggregates:
    ...
    def test_carrier_filter_expects_matching_aggregate_consistency(
        self,
        ...
        with soft_assertions():
            assert_that(int(filtered_expected_aggregate_summary.iloc[0]["result_count"]), "Expected assertion for int(filtered_expected_aggregate_summary.iloc[0]['result_count']) to hold").is_equal_to(1)
            assert_that(filtered_aggregate_comparison_result.is_match, "Expected assertion for filtered_aggregate_comparison_result.is_match to hold").is_true()
            assert_that(filtered_aggregate_comparison_result.summary_mismatches.empty, "Expected assertion for filtered_aggregate_comparison_result.summary_mismatches.empty to hold").is_true()
            assert_that(filtered_aggregate_comparison_result.carrier_count_mismatches.empty, "Expected assertion for filtered_aggregate_comparison_result.carrier_count_mismatches.empty to hold").is_true()
