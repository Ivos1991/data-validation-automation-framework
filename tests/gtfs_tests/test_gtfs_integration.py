...
class TestGtfsIntegration:
    ...
    def test_gtfs_loader_reads_supported_files(self, loaded_gtfs_dataset):
        """Verify the narrow GTFS loader reads the supported file subset."""
        with soft_assertions():
            assert_that(len(loaded_gtfs_dataset.trips_frame)).is_equal_to(4)
            assert_that(len(loaded_gtfs_dataset.calendar_frame)).is_equal_to(3)
            assert_that(len(loaded_gtfs_dataset.stop_times_frame)).is_equal_to(9)
            assert_that(len(loaded_gtfs_dataset.calendar_dates_frame)).is_equal_to(1)
            assert_that(sorted(loaded_gtfs_dataset.routes_frame["route_id"].tolist())).is_equal_to(["R100", "R200", "R300"])

    @allure.title("GTFS transformer derives canonical trip rows across multiple active service dates")
    def test_gtfs_transformer_builds_multidate_canonical_trip_frame(self, gtfs_transformed_trip_frame: pd.DataFrame):
        ...
        with soft_assertions():
            assert_that(len(gtfs_transformed_trip_frame)).is_equal_to(8)
            assert_that(gtfs_transformed_trip_frame.columns.tolist()).is_equal_to(
                ["trip_id", "origin", "destination", "departure_date", "stops_count", "route_id", "carrier", "price_amount", "currency", "duration_minutes"]
            )
            assert_that(len(route_date_slice_20260410)).is_equal_to(2)
            assert_that(len(route_date_slice_20260413)).is_equal_to(2)
            assert_that(sorted(route_date_slice_20260410["carrier"].tolist())).is_equal_to(["AmRail", "BudgetBus"])
            assert_that(sorted(route_date_slice_20260413["carrier"].tolist())).is_equal_to(["AmRail", "BudgetBus"])
            assert_that(sorted(route_date_slice_20260410["stops_count"].tolist())).is_equal_to([0, 1])
            assert_that(trip_001_dates).is_equal_to(["2026-04-10", "2026-04-11", "2026-04-13"])

    @allure.title("GTFS dataset profile resolves matching GTFS assets")
    def test_gtfs_dataset_profile_resolution(self, gtfs_loaded_trip_dataset):
        """Verify the GTFS dataset profile resolves the expected asset set."""
        with soft_assertions():
            assert_that(gtfs_loaded_trip_dataset.dataset_profile).is_equal_to("gtfs")
            assert_that(gtfs_loaded_trip_dataset.trip_dataset_source).is_equal_to("gtfs:data/raw/gtfs_sample")
            assert_that(gtfs_loaded_trip_dataset.scenario_dataset_path.name).is_equal_to("gtfs_batch_trip_search_scenarios.csv")
            assert_that(gtfs_loaded_trip_dataset.default_run_profile_path.name).is_equal_to("gtfs_default_trip_search_run_profile.json")
            assert_that(gtfs_loaded_trip_dataset.default_run_suite_path.name).is_equal_to("gtfs_trip_search_run_suite.json")

    @allure.title("GTFS profile supports deterministic reconciliation across multiple service dates")
    def test_gtfs_profile_search_reconciliation_flow(
        self,
        config,
        ...
        for departure_date in ["2026-04-10", "2026-04-13"]:
            search_filters = {
                "origin": "NYC",
                "destination": "BOS",
            ...
            with soft_assertions():
                assert_that(reconciliation_result.is_match).is_true()
                assert_that(aggregate_result.is_match).is_true()
                assert_that(len(expected_subset)).is_equal_to(2)

    @allure.title("GTFS profile supports aggregate validation on the expanded multi-date dataset")
    def test_gtfs_profile_aggregate_validation_for_multidate_service(
        self,
        config,
        gtfs_loaded_trip_dataset,
        gtfs_trip_search_service_api,
        ...
        with soft_assertions():
            assert_that(aggregate_result.is_match).is_true()
            assert_that(float(build_aggregate_summary(expected_subset).loc[0, "average_price"])).is_equal_to(97.75)

    @allure.title("GTFS profile supports suite execution through the shared suite executor")
    def test_gtfs_profile_suite_execution(
        self,
        config,
        gtfs_loaded_trip_dataset,
        ...
        db_path = db_dir / f"gtfs_suite_{uuid4().hex}.sqlite"
        sqlite_client = SQLiteClient(db_path)
        sqlite_client.initialize_schema()
        TripQueries(sqlite_client).seed_trips(gtfs_loaded_trip_dataset.normalized_trips)
        ...
        try:
            ...
            suite_result = TripSearchRunSuiteExecutor(
                service_api=SearchServiceAPI(TripQueries(sqlite_client)),
            ...
            with soft_assertions():
                assert_that(suite_result.suite_summary_frame.loc[0, "dataset_profile"]).is_equal_to("gtfs")
                assert_that(suite_result.suite_summary_frame.loc[0, "suite_status"]).is_equal_to("passed")
                assert_that(suite_result.suite_summary_frame.loc[0, "scenario_dataset_asset"]).is_equal_to("gtfs_batch_trip_search_scenarios.csv")
    ...
    def test_gtfs_loader_rejects_missing_required_file(self, local_gtfs_test_dir: Path, gtfs_dataset_loader):
        ...
        try:
            try:
                ...
                assert_that(str(error)).contains("stop_times.txt")
            else:
                raise AssertionError("Expected missing GTFS file to fail loading")
        finally:
            for path in missing_file_dir.glob("*"):
                path.unlink()
            missing_file_dir.rmdir()

    @allure.title("GTFS transformer rejects incomplete trip derivation input")
    def test_gtfs_transformer_rejects_incomplete_trip_input(self, loaded_gtfs_dataset, gtfs_trip_transformer: GtfsTripTransformer):
        ...
        try:
            ...
            assert_that(str(error)).contains("origin")
        else:
            raise AssertionError("Expected incomplete GTFS trip derivation to fail")
