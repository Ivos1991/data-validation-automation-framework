...
class TestTripSearchScenarioSelection:
    ...
    def test_scenario_selector_filters_metadata_subsets(
        self,
        ...
        with soft_assertions():
            assert_that([scenario.scenario_id for scenario in filter_pack_scenarios]).is_equal_to(
                ["carrier-filter", "combined-filter"]
            )
            assert_that([scenario.scenario_id for scenario in combined_tag_scenarios]).is_equal_to(
                ["combined-filter", "no-match"]
            )
            assert_that([scenario.scenario_id for scenario in negative_type_scenarios]).is_equal_to(["no-match"])

    @allure.title("Batch validation executes a selected pack subset and reports the selection")
    def test_batch_validation_executes_selected_pack_subset(
        self,
        config,
        ...
        with soft_assertions():
            assert_that(batch_result.summary_frame["scenario_id"].tolist()).is_equal_to(
                ["carrier-filter", "combined-filter"]
            )
            assert_that(batch_result.summary_frame["pack"].tolist()).is_equal_to(["filters", "filters"])
            assert_that(int(run_summary["total_scenarios"])).is_equal_to(2)
            assert_that(run_summary["selected_pack"]).is_equal_to("filters")
            assert_that(run_summary["selected_tag"]).is_equal_to("all")
            assert_that(run_summary["selected_scenario_type"]).is_equal_to("all")
            assert_that(int(batch_result.pack_summary_frame.iloc[0]["total_scenarios"])).is_equal_to(2)

    @allure.title("Batch validation executes a selected tag subset and preserves run summary totals")
    def test_batch_validation_executes_selected_tag_subset(
        self,
        config,
        batch_trip_search_service_api,
        batch_scenario_dataset,
        ...
        with soft_assertions():
            ...
            assert_that(int(run_summary["total_scenarios"])).is_equal_to(2)
            assert_that(run_summary["selected_pack"]).is_equal_to("all")
            assert_that(run_summary["selected_tag"]).is_equal_to("combined")
            assert_that(run_summary["selected_scenario_type"]).is_equal_to("all")

    @allure.title("Batch validation handles empty scenario selections cleanly")
    def test_batch_validation_handles_empty_selection(
        self,
        config,
        batch_trip_search_service_api,
        batch_scenario_dataset,
        ...
        with soft_assertions():
            ...
            assert_that(int(run_summary["total_scenarios"])).is_equal_to(0)
            assert_that(int(run_summary["passed_scenarios"])).is_equal_to(0)
            assert_that(int(run_summary["failed_scenarios"])).is_equal_to(0)
            assert_that(run_summary["selected_pack"]).is_equal_to("missing-pack")

    @allure.title("Batch validation executes subset selection from an external run profile")
    def test_batch_validation_executes_run_profile_subset(
        self,
        config,
        batch_trip_search_service_api,
        batch_scenario_dataset,
        ...
        with soft_assertions():
            ...
            assert_that(run_summary["run_id"]).is_equal_to("filters-pack-run")
            assert_that(run_summary["run_label"]).is_equal_to("Filters Pack")
            assert_that(run_summary["selected_pack"]).is_equal_to("filters")
            assert_that(run_summary["selected_tag"]).is_equal_to("all")
            assert_that(run_summary["selected_scenario_type"]).is_equal_to("all")

    @allure.title("Batch validation handles run profiles that select no scenarios")
    def test_batch_validation_handles_empty_run_profile_selection(
        self,
        config,
        batch_trip_search_service_api,
        batch_scenario_dataset,
        ...
        with soft_assertions():
            assert_that(batch_result.summary_frame.empty).is_true()
            assert_that(int(run_summary["total_scenarios"])).is_equal_to(0)
            assert_that(run_summary["run_id"]).is_equal_to("empty-selection-run")
            assert_that(run_summary["run_label"]).is_equal_to("Empty Selection Run")
            assert_that(run_summary["selected_pack"]).is_equal_to("missing-pack")
